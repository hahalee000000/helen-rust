...
**Objective:** Reimplement `helen/python_bridge/*` so Python code can `from translator import TranslatorAgent` and call Helen agents like Python classes. Strategy: a **PyO3 cdylib** extension (`helen_rust`) built with **maturin** + a thin pure-Python import hook (D9); PyPI distribution name **`helen-rust`**.

## Files

```
crates/helen-python-bridge/Cargo.toml     # [lib] name = "helen_rust", crate-type = ["cdylib", "rlib"]; pyo3 0.23
crates/helen-python-bridge/pyproject.toml # [build-system] maturin; [project] name = "helen-rust"; [tool.maturin] module-name = "helen_rust"
crates/helen-python-bridge/src/lib.rs     // #[pymodule] helen_rust
...
crates/helen-python-bridge/python/helen_rust/import_hook.py  // meta-path finder (pure Python)
```

## Task 11.1: Bridge core (`#[pymodule]`)

Expose to Python:
- `helen_rust.load_agent(file_path: str, agent_name: str) -> PyAgent`
- `helen_rust.load_function(file_path: str, fn_name: str) -> PyFunction`
- `helen_rust.parse_check(source: str) -> list[str]` (semantic error codes, for IDE)
- `helen_rust.eval_helen(source: str, globals: dict) -> object`
...
`PyFunction`: wraps Helen `fn` for direct Python calls (port `function_wrapper.py`). `@helen_agent(file="x.helen", name="Agent")` decorator (port `decorators.py`) — implemented in the Python shim importing from `helen_rust`.

## Task 11.4: Import hook (port `import_hook.py`)

Pure-Python meta-path finder: intercept imports whose module name resolves to a sibling `.helen` file (same dir as the importing module, plus `PYTHONPATH`), parse with `helen_rust`, and inject classes: `__getattr__` on the module returns the agent class (construct → `PyAgent`). Port edge cases: missing file → `ModuleNotFoundError`, parse errors → `SyntaxError` with line info, `.helen` modules without requested agent → `AttributeError`.
