#!/usr/bin/env bash
# GitHub environment detection helper for Helen/Hermes Agent skills.
#
# Usage (via terminal tool):
#   source "${HELEN_HOME:-$HOME/.helen}/skills/devops/github/scripts/gh-env.sh"
#   # or: source "${HERMES_HOME:-$HOME/.hermes}/skills/devops/github/scripts/gh-env.sh"
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _helen_env="${HELEN_HOME:-$HOME/.helen}/.env"; [ -f "$_helen_env" ] && grep -q "^GITHUB_TOKEN=" "$_helen_env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_helen_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
