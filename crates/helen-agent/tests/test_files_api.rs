...
async fn test_upload_file() {
    ...
    let resp = client.post(&format!("{}/api/files", base_url))
        .multipart(form)
        .send()
        .await
        .unwrap();
...
async fn test_download_file() {
    ...
    // Download the file
    let resp = client.get(&format!("{}/api/files/{}", base_url, file_id))
        .send()
        .await
        .unwrap();
...
async fn test_list_files_api() {
    ...
    client.post(&format!("{}/api/files", base_url))
        .multipart(form1)
        .send()
        .await
        .unwrap();
    ...
    // List files
    let resp = client.get(&format!("{}/api/files", base_url))
        .send()
        .await
        .unwrap();
    ...
    assert!(body.len() >= 2, "Expected at least 2 files, got {}", body.len());
}
...
async fn test_delete_file_api() {
    ...
    // Delete the file
    let resp = client.delete(&format!("{}/api/files/{}", base_url, file_id))
        .send()
        .await
        .unwrap();
