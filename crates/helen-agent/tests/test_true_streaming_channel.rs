...
async fn test_streaming_subscription() {
    ...
    assert!(
        bridge.is_alive(),
        "Bridge should be alive after subscription"
    );
}
...
async fn test_send_message() {
    ...
    assert!(
        bridge.is_alive(),
        "Bridge should still be alive after sending message"
    );
}
...
async fn test_streaming_channel_architecture() {
    ...
    assert!(
        bridge.is_alive(),
        "Streaming channel architecture is working"
    );
}
...
async fn test_multiple_subscribers() {
    ...
    assert!(
        stream_rx1.try_recv().is_err(),
        "First subscriber should work"
    );
    assert!(
        stream_rx2.try_recv().is_err(),
        "Second subscriber should work"
    );
}
...
async fn test_bridge_shutdown() {
    ...
    // Bridge should NOT be alive after shutdown
    assert!(
        !bridge.is_alive(),
        "Bridge should not be alive after shutdown"
    );
...
async fn test_heartbeat() {
    ...
    assert!(
        bridge.heartbeat(),
        "Heartbeat should return true for alive bridge"
    );
...
async fn test_stale_detection() {
    ...
    assert!(
        !bridge.is_stale(Duration::from_secs(60)),
        "Bridge should not be stale initially"
    );
    ...
    assert!(
        !bridge.is_stale(Duration::from_secs(60)),
        "Bridge should not be stale after activity"
    );
}
