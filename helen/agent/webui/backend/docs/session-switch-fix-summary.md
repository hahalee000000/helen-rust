...
之前的修复尝试（`websocket-disconnect-persistence.md`）是让 streaming task 独立于 WebSocket 连接，即使 WebSocket 断开也继续运行。但这个设计忽视了：
...
**文件**：`webui/backend/app/routers/chat.py`
...
**文件**：`webui/backend/tests/test_websocket_disconnect.py`

将 `test_streaming_continues_after_disconnect` 改为 `test_streaming_cancelled_on_disconnect`，验证：
- WebSocket 断开后 streaming task 被取消
- 只保存部分结果（到断开时刻）
- 结果标记为 "⚠️ 输出被用户中断"
...
| 场景 | 旧行为（持久化） | 新行为（取消） |
|------|-----------------|---------------|
| 切换会话 | ❌ Transcript 污染 | ✅ 推理中断，部分结果保存 |
| 刷新页面 | ❌ Transcript 污染 | ✅ 推理中断，部分结果保存 |
| 关闭标签页 | ❌ Transcript 污染 | ✅ 推理中断，部分结果保存 |
| 网络断开 | ❌ Transcript 污染 | ✅ 推理中断，部分结果保存 |
...
关键测试：
- `test_streaming_cancelled_on_disconnect`：验证 WebSocket 断开后 streaming task 被取消
- `test_task_cleanup_after_completion`：验证 task 完成后清理
- `test_cancel_still_works`：验证用户主动取消仍然有效

## 文档更新

1. **CLAUDE.md**：更新 WebSocket 断开行为说明
2. **wiki/webui-session-recovery.md**：重写"WebSocket 断开行为"章节
3. **docs/session-switch-transcript-corruption-fix.md**：详细问题分析文档

## 相关文档

- `docs/session-switch-transcript-corruption-fix.md` - 详细问题分析
- `docs/test-database-isolation-fix.md` - 测试数据库隔离修复
- `wiki/webui-session-recovery.md` - Web UI 会话恢复机制
